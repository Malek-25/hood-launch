...
const launcherAbi = ["function launchToken(string,string,uint256,uint256,uint256,address) returns (address,address)","event TokenLaunched(address indexed token,address indexed pair,address indexed tokenOwner,string name,string symbol,uint256 supply)","function launchCount() view returns (uint256)","function launchAt(uint256) view returns (tuple(address token, address pool, address recipient, uint256 supply))","function creator() view returns (address)"];
...
function updatePreview(){ const name=$("tokenName").value.trim()||"Your Token"; const symbol=$("tokenSymbol").value.trim()||"TOKEN"; const icon=$("tokenIcon").value.trim()||"🚀"; $("previewName").textContent=name; $("previewTicker").textContent=symbol; $("previewIcon").textContent=icon; if(name!=="Your Token"||symbol!=="TOKEN"||$("tokenIcon").value.trim()!==""){ $("tokenPreview").classList.remove("hidden"); }else{ $("tokenPreview").classList.add("hidden"); } }
$("tokenName").oninput=updatePreview;
$("tokenSymbol").oninput=updatePreview;
$("tokenIcon").oninput=updatePreview;
$("tokenIcon").onpaste=updatePreview;
// Fetch stats from blockchain
async function loadStats(){ try{ const factAddr=factoryAddress(); if(!isAddress(factAddr))return; const rpcProvider=provider||new BrowserProvider(window.ethereum); const factoryContract=new Contract(factAddr,factoryAbi,rpcProvider); const launcherCount=await factoryContract.launcherCount?.(); if(launcherCount!==undefined){ $("statLaunchers").textContent=launcherCount.toString(); } let totalTokens=0; try{ for(let i=0;i<Math.min(Number(launcherCount),50);i++){ const launcherAddr=await factoryContract.launchers?.(i); if(!launcherAddr)continue; const launcher=new Contract(launcherAddr,["function launchCount() view returns (uint256)"],rpcProvider); const count=await launcher.launchCount?.(); if(count!==undefined)totalTokens+=Number(count); } $("statTokens").textContent=totalTokens.toString(); }catch(e){console.error("Token count error:",e);} }catch(e){ console.error("Failed to load stats:",e); } }
// Load stats on page load
if(window.ethereum){ loadStats(); }
// Tab switching for explore
let allLaunchersData=[];let allTokensData=[];
document.querySelectorAll(".explore-tab").forEach(tab=>{tab.onclick=()=>{document.querySelectorAll(".explore-tab").forEach(t=>t.classList.remove("active"));tab.classList.add("active");const tabName=tab.dataset.tab;if(tabName==="all"){$("exploreAll").style.display="block";$("exploreMine").style.display="none";}else{$("exploreAll").style.display="none";$("exploreMine").style.display="block";filterMyLaunches();}};});
function filterMyLaunches(){if(!account){$("myLaunchersList").innerHTML="<p style='color:var(--muted);font-size:13px;padding:12px;'>Connect your wallet to see your launches.</p>";$("myTokensList").innerHTML="";return;}const myLaunchers=allLaunchersData.filter(l=>l.creator?.toLowerCase()===account.toLowerCase());const myTokens=allTokensData.filter(t=>t.creator?.toLowerCase()===account.toLowerCase());$("myLaunchersList").innerHTML=myLaunchers.length?myLaunchers.map(l=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);display:flex;justify-content:space-between;align-items:center;border-radius:4px;"><div><strong style="font-size:14px;">${l.name||'Launcher #'+(l.index+1)}</strong><br><span style="font:11px 'DM Mono',monospace;color:var(--muted);">${l.addr}</span></div><a href="https://robinhoodchain.blockscout.com/address/${l.addr}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:12px;">View ↗</a></div>`).join(""):"<p style='color:var(--muted);font-size:13px;padding:12px;'>You haven't created any launchers yet.</p>";$("myTokensList").innerHTML=myTokens.length?myTokens.map(t=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);border-radius:4px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font:11px 'DM Mono',monospace;color:var(--muted);">Token: ${t.token.slice(0,10)}…${t.token.slice(-8)}</span><a href="https://robinhoodchain.blockscout.com/address/${t.token}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:11px;">View ↗</a></div><div style="font:11px 'DM Mono',monospace;color:var(--muted);">$HOODIE Pool: ${t.pool.slice(0,10)}…${t.pool.slice(-8)} <a href="https://robinhoodchain.blockscout.com/address/${t.pool}" target="_blank" style="color:var(--acid);">↗</a></div></div>`).join(""):"<p style='color:var(--muted);font-size:13px;padding:12px;'>You haven't launched any tokens yet.</p>";}
// Explore feature - load recent launchers and tokens
$("loadExplore").onclick=async()=>{try{$("loadExplore").textContent="Loading...";const factAddr=factoryAddress();if(!isAddress(factAddr))throw new Error("Factory not configured");const rpcProvider=provider||new BrowserProvider(window.ethereum);const factoryContract=new Contract(factAddr,factoryAbi,rpcProvider);const count=await factoryContract.launcherCount();const limit=Math.min(Number(count),10);allLaunchersData=[];for(let i=Number(count)-1;i>=Math.max(0,Number(count)-limit);i--){const addr=await factoryContract.launchers(i);try{const launcherContract=new Contract(addr,launcherAbi,rpcProvider);const creator=await launcherContract.creator();allLaunchersData.push({addr,index:i,creator});}catch{allLaunchersData.push({addr,index:i,creator:null});}}$("launchersList").innerHTML=allLaunchersData.map(l=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);display:flex;justify-content:space-between;align-items:center;border-radius:4px;"><div><strong style="font-size:14px;">Launcher #${l.index+1}</strong><br><span style="font:11px 'DM Mono',monospace;color:var(--muted);">${l.addr}</span></div><a href="https://robinhoodchain.blockscout.com/address/${l.addr}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:12px;">View ↗</a></div>`).join("");allTokensData=[];for(const l of allLaunchersData.slice(0,5)){try{const launcherContract=new Contract(l.addr,launcherAbi,rpcProvider);const tokenCount=await launcherContract.launchCount();for(let j=0;j<Math.min(Number(tokenCount),3);j++){const launch=await launcherContract.launchAt(j);allTokensData.push({token:launch.token,pool:launch.pool,launcher:l.addr,creator:launch.recipient});}}catch{}}$("tokensList").innerHTML=allTokensData.length?allTokensData.map(t=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);border-radius:4px;"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font:11px 'DM Mono',monospace;color:var(--muted);">Token: ${t.token.slice(0,10)}…${t.token.slice(-8)}</span><a href="https://robinhoodchain.blockscout.com/address/${t.token}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:11px;">View ↗</a></div><div style="font:11px 'DM Mono',monospace;color:var(--muted);">$HOODIE Pool: ${t.pool.slice(0,10)}…${t.pool.slice(-8)} <a href="https://robinhoodchain.blockscout.com/address/${t.pool}" target="_blank" style="color:var(--acid);">↗</a></div></div>`).join(""):"<p style='color:var(--muted);font-size:13px;padding:12px;'>No tokens launched yet. Be the first!</p>";$("loadExplore").textContent="Refresh";$("loadExplore").style.background="var(--acid)";$("loadExplore").style.color="var(--ink)";if(account){$("myTab").style.display="block";}}catch(e){$("loadExplore").textContent="Load recent launches";console.error("Explore error:",e);toast(e.message);}};
