...
$("loadExplore").onclick=async()=>{try{$("loadExplore").textContent="Loading...";const factAddr=factoryAddress();if(!isAddress(factAddr))throw new Error("Factory not configured");const rpcProvider=provider||new BrowserProvider(window.ethereum);const factoryContract=new Contract(factAddr,factoryAbi,rpcProvider);const count=await factoryContract.launcherCount();const limit=Math.min(Number(count),10);const launchers=[];for(let i=Number(count)-1;i>=Math.max(0,Number(count)-limit);i--){const addr=await factoryContract.launchers(i);const creatorList=await factoryContract.launchersFor?.(addr);launchers.push({addr,index:i});}$("launchersList").innerHTML=launchers.map(l=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);display:flex;justify-content:space-between;align-items:center;"><div><strong style="font-size:14px;">Launcher #${l.index+1}</strong><br><span style="font:11px 'DM Mono',monospace;color:var(--muted);">${l.addr}</span></div><a href="https://robinhoodchain.blockscout.com/address/${l.addr}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:12px;">View ↗</a></div>`).join("");let allTokens=[];for(const l of launchers.slice(0,5)){try{const launcherContract=new Contract(l.addr,["function launchCount() view returns (uint256)","function launchAt(uint256) view returns (tuple(address token, address pool, address recipient, uint256 supply))"],rpcProvider);const tokenCount=await launcherContract.launchCount();for(let j=0;j<Math.min(Number(tokenCount),3);j++){const launch=await launcherContract.launchAt(j);allTokens.push({token:launch.token,pool:launch.pool,launcher:l.addr});}}catch{}}$("tokensList").innerHTML=allTokens.length?allTokens.map(t=>`<div style="padding:12px;background:#f8f9f5;border:1px solid var(--line);"><div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span style="font:11px 'DM Mono',monospace;color:var(--muted);">Token: ${t.token.slice(0,10)}…${t.token.slice(-8)}</span><a href="https://robinhoodchain.blockscout.com/address/${t.token}" target="_blank" style="color:var(--acid);text-decoration:none;font-size:11px;">View ↗</a></div><div style="font:11px 'DM Mono',monospace;color:var(--muted);">$HOODIE Pool: ${t.pool.slice(0,10)}…${t.pool.slice(-8)} <a href="https://robinhoodchain.blockscout.com/address/${t.pool}" target="_blank" style="color:var(--acid);">↗</a></div></div>`).join(""):"<p style='color:var(--muted);font-size:13px;'>No tokens launched yet. Be the first!</p>";$("loadExplore").textContent="Refresh";$("loadExplore").style.background="var(--acid)";$("loadExplore").style.color="var(--ink)";}catch(e){$("loadExplore").textContent="Load recent launches";toast(e.message);}};
